...
"""
V8.20 — Predictive Fitness Evolution (Exploration Reward + Penta Memory + Seed V8.18)
Fitness = 15% Luc + 85% R² + Diversity + Exploration Reward
"""
...
PRED_LAGS = [70, 50, 30, 10]   # Penta memory
BASE_ADAPT_NOISE = 0.05
NOISE_INCREASE = 0.001
VALID_STEPS = 450
...
EXPLORATION_REWARD = 0.08

BASE_MUT_RATE = 0.05
MUT_DECAY = 0.0005

# META_BOUNDS (copié de V8.13 pour adaptive_mutate)
META_BOUNDS = {
    "kappa_gain":     (0.01, 2.0),
...
def load_previous_population(file="final_population_v8_18.npz"):
...
def create_population(pop_size):
    ...
    print(f"   ✅ Seed chargé : final_population_v8_18.npz ({len(seed_pop)} individus)")
...
def compute_predictive_r2(ind, gen):
    try:
        ...
        # Exploration reward (bonus si long lag meilleur que court)
        short_avg = np.mean(r2s_all[-2:])   # derniers lags = courts
        long_r2 = r2s_all[0]                # premier = lag 70
        exp_bonus = EXPLORATION_REWARD if long_r2 > short_avg else 0.0
...
def run_evolution():
    print("🚀 LANCEMENT V8.20 — EXPLORATION REWARD + PENTA MEMORY")
    print(f"   PRED_LAGS={PRED_LAGS} | ADAPT_NOISE croissant | Seed V8.18\n")
    ...
    np.savez("final_population_v8_20.npz", population=pop, history=history)
    print("\n✅ V8.20 TERMINÉ")
    print("   Fichier final : final_population_v8_20.npz")
