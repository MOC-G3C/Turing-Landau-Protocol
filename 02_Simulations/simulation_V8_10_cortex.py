...
# ============================================================
# 1. CONSTANTES PHYSIQUES ET HYPERPARAMÈTRES V8.10 (CORTEX)
# ============================================================
...
# Note: "tau" a été retiré des META_BOUNDS, il devient une matrice spatiale.
META_BOUNDS = {
    "kappa_gain":       (0.01, 2.0),
...
_metric_history = {
    ...
    "luc":   deque(maxlen=ZSCORE_WINDOW) 
}
...
def creer_individu_aleatoire(shape=(16, 16)):
    return {
        ...
        # NOUVEAU : La matrice de mémoire spatiale (initialisée uniformément pour forcer l'évolution à la sculpter)
...
def eval_individu_raw(adn, steps=200):
    ...
    for _ in range(3):
        ...
        tau_mat = adn["tau_matrix"].copy() # NOUVEAU: matrice de relaxation spatiale
        meta = adn["meta"]
        ...
        h = np.zeros_like(theta) # NOUVEAU: la perception de l'environnement est spatiale
        ...
        for t in range(1, steps + 1):
            ...
            # Chaque neurone filtre l'environnement avec son propre taux de mémoire
            h = tau_mat * h + (1.0 - tau_mat) * env_val

            var         = float(np.var(theta))
            mean_so_far = float(np.mean(theta_means)) if theta_means else 0.0
            
            # L'environnement global est la moyenne des perceptions locales
            ...
            is_spike = abs(float(np.mean(h))) > 2.0
            ...
            # === Forgetting Spatial (Taxe de Landauer locale) ===
    ...
    return {
        ...
        "tau_std":       float(np.std(adn["tau_matrix"])) # Clé : La différenciation
    }
...
def croiser_adn(adn1, adn2):
    ...
    new_tau   = np.where(mask, adn1["tau_matrix"], adn2["tau_matrix"]) # Croisement spatial de la mémoire
...
def muter_adn(adn, mutation_rate=0.1):
    ...
    # Mutation de la matrice Tau (dérive génétique locale)
    tau_mutation = np.random.randn(16, 16) * (mutation_rate * 0.5)
    new_tau = np.clip(adn["tau_matrix"] + tau_mutation, 0.01, 0.99)
...
def run_evolution(pop_size=40, generations=20):
    ...
    print(f"🔬 Début Évolution V8.10 (Émergence Corticale) - {generations} Gen | Pop: {pop_size}")
    ...
    for gen in range(generations):
        ...
        # Tracking de la matrice Tau
        tau_mean_pop = np.mean([e["tau_mean"] for e in evals])
        tau_std_pop  = np.mean([e["tau_std"] for e in evals]) # Si ça monte, l'anatomie se forme !
        
        idx = np.argsort(probs)[::-1]
        best_idx = idx[0]
        fit_std = float(np.std(probs))

        print(f"Gen {gen:03d} | Surv: {surv_pct:5.1f}% | Luc: {luc_mean:.2f} | Q: {q_landauer_mean:.2e} | Tau_M: {tau_mean_pop:.3f} | Tau_Std (Différenciation): {tau_std_pop:.3f}")
    ...
    # Export du Master DNA et affichage de l'anatomie
    ...
    champion = population[idx_final[0]]
    champion_tau = champion["tau_matrix"]
    
    print("\n🧠 CARTOGRAPHIE DU CORTEX (Matrice Tau du Champion) :")
    ...
    np.savez_compressed("final_population_v8_10.npz", population=np.array(population_triee, dtype=object))
    print("\n💾 Population finale sauvegardée dans 'final_population_v8_10.npz'")
