...
"""
V8.22 — Predictive Fitness Evolution (Anti-Homogénéisation + Immigration)
...
# ===============================
# SEED AVEC FALLBACK ROBUSTE
# ===============================

def load_previous_population():
    for fname in ["final_population_v8_21.npz", "final_population_v8_20.npz", "final_population_v8_18.npz"]:
        try:
            data = np.load(fname, allow_pickle=True)
            print(f"   ✅ Seed chargé : {fname} ({len(data['population'])} individus)")
            return list(data['population'])
        except FileNotFoundError:
            continue
    print("   ⚠️  Aucun seed trouvé → création d'une nouvelle population")
    return [engine.creer_individu_aleatoire() for _ in range(40)]
...
def compute_predictive_r2(ind, gen):
    try:
        ...
        long_r2 = r2s_all[0]
        short_avg = np.mean(r2s_all[-3:])
        exp_bonus = EXPLORATION_REWARD if long_r2 > short_avg * 1.08 else 0.0
...
def run_evolution():
    print("🚀 LANCEMENT V8.22 — ANTI-HOMOGÉNÉISATION + IMMIGRATION")
    print(f"   PRED_LAGS={PRED_LAGS} | Elite=3 + Immigration tous les 25 gens\n")
    ...
    for gen in range(GENS):
        ...
        # Immigration tous les 25 générations
        if gen % 25 == 0 and gen > 0:
            immigrants = [engine.creer_individu_aleatoire() for _ in range(4)]
            for i in range(4):
                pop[order[-1-i]] = copy.deepcopy(immigrants[i])
            print(f"   🌍 Immigration : 4 nouveaux individus injectés")

        survivors = [copy.deepcopy(evals[i][0]) for i in order[:POP//2]]
        newpop = [copy.deepcopy(evals[i][0]) for i in order[:3]]  # Elite réduit à 3

        while len(newpop) < POP:
            if np.random.rand() < 0.22:  # crossover réduit
    ...
    np.savez("final_population_v8_22.npz", population=pop, history=history)
    print("\n✅ V8.22 TERMINÉ")
    print("   Fichier final : final_population_v8_22.npz")
