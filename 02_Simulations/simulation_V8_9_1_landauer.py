...
# ============================================================
# 1. CONSTANTES PHYSIQUES ET HYPERPARAMÈTRES V8.9.1
# ============================================================
...
# --- NOUVELLES CONSTANTES LANDAUER / LUCIDITÉ (V8.9.1) ---
INFO_TO_BITS = 0.2         # Moins punitif sur chaque bit (auparavant 0.5)
LN2 = math.log(2.0)

# L'échelle Thermique est maintenant alignée sur la réalité quantique de la simulation
Q_soft_min = 1e-20         # Début de l'inconfort thermique
Q_MAX_LANDAUER = 5e-19     # Plafond de verre thermique (x10 par rapport aux morts de la V8.9)

SEUIL_REACTION = 0.08      # Pression cognitive accrue (exige une forte réaction)
MAX_INACTION = 3           # Tolérance d'ignorance avant mort cognitive
...
# ============================================================
# 3. MOTEUR DE SIMULATION V8.9.1 (L'Arène)
# ============================================================
def eval_individu_raw(adn, steps=200):
    ...
    for _ in range(3):
        ...
        survival_time = 0
        consecutive_low = 0
        low_var_grace = 150 # <-- V8.9.1 : Désactivation de la mort par ennui (était à 15)
        h = 0.0
        
        # --- Initialisation Landauer / Lucidité ---
        Q_heat = 0.0
        lucidity_score = 0.0
        ...
        for t in range(1, steps + 1):
            ...
            # === Forgetting / Relaxation (Taxe de Landauer) ===
            ...
            # Vérification variance trop faible (beaucoup plus permissive maintenant)
    ...
    return {
        ...
        "tau":           meta["tau"] # On remonte tau pour l'analyse
    }
...
def compute_population_fitness(evals):
    ...
    for idx, e in enumerate(evals):
        if e["inaction_avg"] > MAX_INACTION:
            raw_fits[idx] = 0.01 # Mort cognitive
        else:
...
def run_evolution(pop_size=40, generations=20):
    ...
    print(f"🔬 Début Évolution V8.9.1 (Landauer Strict + Survie Macro) - {generations} Gen | Pop: {pop_size}")

    for gen in range(generations):
        evals = [eval_individu_raw(ind) for ind in population]
        probs = compute_population_fitness(evals)
        
        # Tracking métriques V8.9.1
        ...
        tau_mean = np.mean([e["tau"] for e in evals]) # Observation critique de l'émergence de la mémoire
        
        idx = np.argsort(probs)[::-1]
        best_idx = idx[0]
        fit_std = float(np.std(probs))

        print(f"Gen {gen:03d} | Max Fit: {probs[best_idx]:.4f} | Surv: {surv_pct:5.1f}% | Luc: {luc_mean:.2f} | Inact: {ina_mean:.2f} | Q: {q_landauer_mean:.2e} | Tau (Mémoire): {tau_mean:.3f}")

        # Validation sur le champion
    ...
    np.savez_compressed("final_population_v8_9_1.npz", population=np.array(population_triee, dtype=object))
    print("💾 Population finale sauvegardée dans 'final_population_v8_9_1.npz'")
